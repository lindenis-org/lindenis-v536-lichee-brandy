...
int update_fdt_para_for_kernel(void *dtb_base)
{
	...
	case STORAGE_SD: {
		...
		if (dragonboard_test == 1) {
			...
			ret = script_parser_fetch_ex("target", "lind_test_root", (int*)name, &ret_type,(sizeof(script_gpio_set_t)));

			if (ret == SCRIPT_PARSER_OK) {
				setenv("mmc_root", name);
			} else {
				pr_warning("parse lind_test_root failed! using default root! \n");
				setenv("mmc_root", "/dev/mmcblk1p7");
			}
		} else {
			disable_node("nand0");
			disable_node("mmc2");
...
int update_dram_para_for_ota(void)
{
	...
	if ((get_boot_work_mode() == WORK_MODE_BOOT) &&
		...
		if (get_boot_dram_update_flag() ||
		    mmc_request_update_boot0(card_num)) {

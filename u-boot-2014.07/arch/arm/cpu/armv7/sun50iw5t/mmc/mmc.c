	...
	if (IS_SD(mmc)) {
		...
		if (mmc->card_caps & MMC_MODE_8BIT) {
			...
			if ((mmc->card_caps & MMC_MODE_DDR_52MHz)) {
				...
				if (err) {
					mmcinfo(
					    "mmc %d switch bus width8 failed\n",
					    mmc->control_num);
					return err;
				}
				mmc_set_bus_width(mmc, 8);
...
int mmc_init(struct mmc *mmc)
{
	...
	if (priv_info->card_type == CARD_TYPE_SD) {
		...
		if (err) {
			mmcinfo("SD card %d Card did not respond to voltage "
				"select!\n",
				mmc->control_num);
			mmcinfo("***SD/MMC %d init error!!!***\n",
				mmc->control_num);
		...
		if (err) {
			mmcinfo("MMC card %d Card did not respond to voltage "
				"select!\n",
				mmc->control_num);
			mmcinfo("***SD/MMC %d init error!!!***\n",
				mmc->control_num);
		...
		if (err) {
			...
			if (err) {
				mmcinfo("mmc %d Card did not respond to "
					"voltage select!\n",
					mmc->control_num);
				mmcinfo("***SD/MMC %d init error!!!***\n",
					mmc->control_num);
